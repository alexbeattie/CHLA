...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            san_gabriel = RegionalCenter.objects.filter(regional_center__icontains='san gabriel').first()
            if san_gabriel:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Updated San Gabriel/Pomona Regional Center with {len(san_gabriel_zip_codes)} ZIP codes')
                )
                self.stdout.write(f'ZIP 91769 (Pomona) included: {"91769" in san_gabriel.zip_codes}')
                self.stdout.write(f'ZIP 91776 (San Gabriel) included: {"91776" in san_gabriel.zip_codes}')
            else:
                self.stdout.write(self.style.ERROR('San Gabriel/Pomona Regional Center not found'))
