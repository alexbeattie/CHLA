...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options["providers"] and not options["regional_centers"]:
            self.stdout.write(
                self.style.ERROR(
                    "Please specify --providers, --regional-centers, or --all"
                )
            )
            return
    ...
    def geocode_providers(self, force=False, dry_run=False):
        ...
        if force:
            ...
            providers = ProviderV2.objects.filter(
                ...
                | Q(latitude=Decimal("0.00000000"))
                | Q(longitude=Decimal("0.00000000"))
            )
        ...
        if dry_run:
            ...
            for provider in providers[:10]:  # Show first 10
                self.stdout.write(
                    f"Would geocode: {provider.name} - {provider.address}"
                )
        ...
        for i, provider in enumerate(providers, 1):
            ...
            try:
                ...
                if coordinates:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  ✓ Success: {coordinates[0]}, {coordinates[1]}"
                        )
                    )
                else:
                    failed_count += 1
    ...
    def geocode_regional_centers(self, force=False, dry_run=False):
        ...
        if dry_run:
            ...
            for center in centers[:10]:  # Show first 10
                self.stdout.write(
                    f"Would geocode: {center.regional_center} - {center.address}"
                )
            if total_centers > 10:
        ...
        for i, center in enumerate(centers, 1):
            self.stdout.write(
                f"[{i}/{total_centers}] Geocoding {center.regional_center}..."
            )
