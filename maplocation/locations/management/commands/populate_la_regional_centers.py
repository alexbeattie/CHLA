        ...
        for center_name, center_data in la_regional_centers.items():
            try:
                ...
                existing_centers = RegionalCenter.objects.filter(
                    regional_center__icontains=center_name.split()[
                        0
                    ]  # First word of name
                ).filter(
                    regional_center__icontains=center_name.split()[
                ...
                if existing_centers.exists():
                    ...
                    self.stdout.write(
                        f"Found existing record for {center_name}: ID {regional_center.id}"
                    )
                else:
                    # Create new record
                ...
                self.stdout.write(
                    self.style.ERROR(f"✗ Error processing {center_name}: {e}")
                )
