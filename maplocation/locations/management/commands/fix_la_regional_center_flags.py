...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for rc_name in self.LA_COUNTY_REGIONAL_CENTERS:
            ...
            if updated > 0:
                ...
                if count > 1:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"  ✅ UPDATED: {rc_name}"))

        self.stdout.write("")
        self.stdout.write(
            self.style.SUCCESS(f"✨ Updated {updated_count} Regional Center record(s)")
        )
        ...
        self.stdout.write(
            f"  LA County RCs (is_la_regional_center=True): {la_county_rcs}"
        )
