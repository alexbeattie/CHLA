...
class CSVExporter:
    ...
    def export_providers(queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename="providers_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
    ...
    def export_regional_centers(queryset):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="regional_centers_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class CSVImporter:
    ...
    def _associate_regional_centers(self, provider, regional_centers_str, row_num):
        ...
        regional_center_names = [name.strip() for name in regional_centers_str.split(',')]
            ...
            with transaction.atomic():
                for row_num, row in enumerate(reader, start=2):  # Start at 2 (header is row 1)
                    try:
                        ...
                        self.error_count += 1
                        self.errors.append(f"Row {row_num}: {str(e)}")
                        logger.error(f"Error processing regional center row {row_num}: {e}")
    ...
    def _process_regional_center_row(self, row, row_num, update_existing):
        ...
        if update_existing:
            existing_center = RegionalCenter.objects.filter(regional_center=name).first()
