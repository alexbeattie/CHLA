...
from django.forms import TextInput, Textarea, ModelForm, SelectMultiple, CheckboxSelectMultiple, MultipleChoiceField
...
class ProviderV2Form(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'insurance_accepted': Textarea(attrs={'rows': 3, 'cols': 60, 'readonly': True}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            if insurance_text:
                ...
                selected_insurances = []
                for item in insurance_text.replace(',', '|').replace(';', '|').split('|'):
                    item = item.strip().strip('"\'{}[]')
                    if item and item in [choice[0] for choice in ProviderV2.INSURANCE_CHOICES]:
                ...
                if field_value:
                    if isinstance(field_value, str):
                        try:
                            ...
                            self.fields[form_field].initial = [field_value] if field_value else []
                    else:
    ...
    def clean(self):
        ...
        if insurance_selection:
            ...
            self.add_error('insurance_selection', 'Please select at least one insurance type.')
        ...
        # Handle JSON fields
        json_fields = ['hours', 'age_groups', 'diagnoses_treated', 'therapy_types', 'funding_sources', 'specific_areas_served']
        ...
        if address and (lat in (None, '') or lng in (None, '')):
            ...
            if coords:
                ...
                if not lat:
                    self.add_error('latitude', 'Please provide latitude or use a more specific address that can be geocoded.')
                if not lng:
                    self.add_error('longitude', 'Please provide longitude or use a more specific address that can be geocoded.')
        else:
    ...
    def save(self, commit=True):
        ...
        print(f"DEBUG: insurance_selection: {self.cleaned_data.get('insurance_selection', [])}")
        print(f"DEBUG: age_groups_selection: {self.cleaned_data.get('age_groups_selection', [])}")
        print(f"DEBUG: diagnoses_selection: {self.cleaned_data.get('diagnoses_selection', [])}")
        print(f"DEBUG: therapy_types_selection: {self.cleaned_data.get('therapy_types_selection', [])}")
...
class ProviderV2Admin(admin.ModelAdmin):
    form = ProviderV2Form
    list_display = ['name', 'type', 'phone', 'verified', 'get_age_groups', 'get_diagnoses', 'get_therapy_types']
    ...
    fieldsets = (
        ...
        ('Funding & Payment', {
            'fields': ('funding_sources', 'accepts_private_pay', 'accepts_regional_center', 'accepts_insurance'),
            'description': 'Funding sources and payment options accepted',
        }),
        ('Service Delivery', {
            'fields': ('in_person_services', 'virtual_services', 'home_based_services', 'center_based_services'),
        ...
        ('Service Area', {
            'fields': ('service_radius_miles', 'serves_la_county', 'specific_areas_served'),
