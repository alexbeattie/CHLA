...
class LocationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Contact Information", {"fields": ("phone", "website", "email")}),
        (
            "Address",
            {
                "fields": (
...
class ProviderV2Form(ModelForm):
    ...
    def clean(self):
        ...
        cleaned["insurance_accepted"] = normalize_text_or_json(
            cleaned.get("insurance_accepted")
        )
...
class ProviderV2Admin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        "phone",
        "verified",
    ]  # Simplified to avoid city/state methods
    ...
    readonly_fields = ["id", "created_at", "updated_at"]  # Removed coordinates_display
    ...
    fieldsets = (
        ...
        ("Address & Location", {"fields": ("address", "latitude", "longitude")}),
        (
            "Details",
            {
    ...
    # Removed coordinate methods to avoid errors
...
class ProviderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        "created_display",
        "coordinates_display",
        "regional_centers_display",
    ]
    inlines = [
        ProviderRegionalCenterInline,
        ProviderFundingSourceInline,
        ProviderInsuranceCarrierInline,
        ProviderServiceModelInline,
    ]
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "phone", "website_domain")}),
        (
            "Address & Location",
            {
    ...
    def regional_centers_display(self, obj):
        ...
        if centers.exists():
            ...
            for rel in centers:
                url = reverse(
                    "admin:locations_regionalcenter_change",
                    args=[rel.regional_center.id],
                )
                links.append(
                    format_html(
                        '<a href="{}">{}</a>', url, rel.regional_center.regional_center
                    )
                )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path(
                ...
                name="provider_download_template",
            ),
        ]
        ...
        if request.method == "POST":
            ...
            if csv_file:
                ...
                if result["error_count"] > 0:
                    messages.error(
                        request, f'Failed to import {result["error_count"]} providers'
                    )
    ...
    def download_template(self, request):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="provider_import_template.csv"'
        )
...
class RegionalCenterAdmin(admin.ModelAdmin):
    ...
    def provider_count_display(self, obj):
        ...
        if count > 0:
            return format_html(
                "{} providers associated with this regional center", count
            )
