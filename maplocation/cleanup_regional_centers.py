...
def analyze_duplicates():
    ...
    duplicates = {name: centers for name, centers in name_groups.items() if len(centers) > 1}
    ...
    for name, centers in duplicates.items():
        ...
        for office_type, type_centers in office_types.items():
            ...
            if len(type_centers) == 1:
                ...
                print(f"    ✅ Keep: ID {type_centers[0].id} ({type_centers[0].address})")
            else:
                ...
                # Sort by priority
                sorted_centers = sorted(type_centers, key=lambda x: (
                    priority_order.index(x.office_type) if x.office_type in priority_order else 999,
                ...
                print(f"    ✅ Keep: ID {sorted_centers[0].id} ({sorted_centers[0].address})")
