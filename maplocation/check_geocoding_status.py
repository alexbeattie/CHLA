...
providers_without_coords = ProviderV2.objects.filter(
    latitude__isnull=True, longitude__isnull=True
).count() + ProviderV2.objects.filter(
    latitude=Decimal('0.00000000'), longitude=Decimal('0.00000000')
).count()
providers_with_coords = ProviderV2.objects.exclude(
...
centers_without_coords = RegionalCenter.objects.filter(
    latitude__isnull=True, longitude__isnull=True
).count() + RegionalCenter.objects.filter(
    latitude=Decimal('0.00000000'), longitude=Decimal('0.00000000')
).count()
centers_with_coords = RegionalCenter.objects.exclude(
    latitude__isnull=True
).exclude(
    longitude__isnull=True
).exclude(
...
if providers_without_coords > 0:
    ...
    providers_needing_geocoding = list(ProviderV2.objects.filter(
        latitude__isnull=True, longitude__isnull=True
    )[:3]) + list(ProviderV2.objects.filter(
        latitude=Decimal('0.00000000'), longitude=Decimal('0.00000000')
    )[:3])
    
    for provider in providers_needing_geocoding[:5]:
        print(f"  - {provider.name} (lat: {provider.latitude}, lon: {provider.longitude})")
...
if centers_without_coords > 0:
    ...
    centers_needing_geocoding = list(RegionalCenter.objects.filter(
        latitude__isnull=True, longitude__isnull=True
    )[:3]) + list(RegionalCenter.objects.filter(
        latitude=Decimal('0.00000000'), longitude=Decimal('0.00000000')
    )[:3])
    
    for center in centers_needing_geocoding[:5]:
        print(f"  - {center.regional_center} (lat: {center.latitude}, lon: {center.longitude})")
