...
import { sampleCategories, sampleLocations, sampleUserProfile } from '@/assets/sampleData';
...
export default {
  ...
  computed: {
    ...
    filteredLocations() {
      ...
      return this.locations.filter(location => {
        // Category filter
        if (this.selectedCategory && String(location.category) !== String(this.selectedCategory)) {
          return false;
        }
  ...
  methods: {
    ...
    initMap() {
      ...
      mapboxgl.accessToken = import.meta.env.VITE_MAPBOX_TOKEN || 'pk.eyJ1IjoiYWxleGJlYXR0aWUiLCJhIjoiOVVEYU52WSJ9.S_uekMjvfZC5_s0dVVJgQg';
      ...
      try {
        ...
        try {
          ...
          if (response.data && Array.isArray(response.data)) {
            ...
            this.categories = response.data.results.filter(category => category && category.id);
            console.log(`Loaded ${this.categories.length} categories from paginated results`);
          } else {
            console.warn('Invalid categories data format:', response.data);
            this.categories = sampleCategories;
          }
        } catch (apiError) {
          console.error('Error fetching categories from API, using sample categories:', apiError);
          this.categories = sampleCategories;
        }
      } catch (error) {
    ...
    async fetchAllLocations() {
      ...
      try {
        ...
        try {
          ...
          console.error('Error fetching locations from API, using sample data:', apiError);
          this.locations = sampleLocations;
        }
    ...
    async fetchNearbyLocations() {
      ...
      try {
        console.log(`Fetching nearby locations with lat=${this.userLocation.latitude}, lng=${this.userLocation.longitude}, radius=${this.radius}km`);
        
        // Use sample data with calculated distances if configured to bypass API
        if (USE_LOCAL_DATA_ONLY) {
          console.log('Using sample locations data with distance calculation (API calls disabled)');
          
          // Calculate distances for all sample locations
          this.locations = sampleLocations.map(location => {
            ...
            const distance = Math.sqrt(
              ...
              Math.pow((locationLng - userLng) * 111 * Math.cos(userLat * (Math.PI/180)), 2)
            ).toFixed(1);
          ...
          console.log(`Calculated distances for ${this.locations.length} locations within ${this.radius}km radius`);
          this.loading = false;
          return;
        }
        ...
        try {
          ...
          console.error('Error fetching nearby locations from API, using sample data with distance calculation:', apiError);
      ...
      this.filteredLocations.forEach(location => {
        try {
          ...
          if (!location.latitude || !location.longitude) {
            console.warn(`Location ${location.id} is missing coordinates, skipping marker`);
            return;
          }
          ...
          try {
            ...
            if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
              console.warn(`Location ${location.id} has invalid coordinates: ${lat}, ${lng}`);
              return;
            }
          } catch (e) {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            // Check for valid coordinates
            if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
              console.warn(`Cannot center map: Location ${location.id} has invalid coordinates: ${lat}, ${lng}`);
              return;
            }
    ...
    saveUserData(userData) {
      ...
      if (USE_LOCAL_DATA_ONLY) {
        ...
        if (this.userData.address && (!this.userLocation.latitude || !this.userLocation.longitude)) {
      ...
      axios.post('/api/users/profiles/update_me/', {
        age: this.userData.age ? parseInt(this.userData.age) : null,
        address: this.userData.address,
        diagnosis: this.userData.diagnosis.toLowerCase(),
        other_diagnosis: this.userData.diagnosis === 'Other' ? this.userData.otherDiagnosis : ''
      })
      .then(response => {
      ...
      .catch(error => {
        ...
        alert('There was an issue saving to the server, but your data has been saved locally.');
      ...
      axios.get(geocodingUrl)
        .then(response => {
          ...
          if (response.data.features && response.data.features.length > 0) {
            ...
            console.warn('No geocoding results found for address:', this.userData.address);
          }
        })
        .catch(error => {
    ...
    loadUserData() {
      ...
      try {
        axios.get('/api/users/profiles/me/')
          .then(response => {
          ...
          .catch(error => {
            console.warn('Could not load user profile from API, falling back to localStorage:', error);
