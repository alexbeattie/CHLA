# KiNDD App Store Submission - Version 1.3.0

## Version Information
- **Marketing Version:** 1.3.0
- **Build Number:** 1
- **Minimum iOS:** 17.0

---

## What's New (Release Notes)

### English
```
Version 1.3.0 - Spanish Language Support & Enhanced Navigation

NEW FEATURES:
• Full Spanish language support - Switch between English and Español in Settings or from the home screen
• AI assistant now responds in your preferred language
• Improved Regional Centers navigation with Map/List toggle
• Quick access to About & FAQ directly from home screen
• Redesigned home page with cleaner layout

IMPROVEMENTS:
• Better localization throughout the app
• Streamlined navigation to key information
• Enhanced map view as default for Regional Centers
• Improved accessibility for Spanish-speaking families

Thank you for using KiNDD to find developmental disability services in Los Angeles County!
```

### Spanish (Español)
```
Versión 1.3.0 - Soporte en Español y Navegación Mejorada

NUEVAS FUNCIONES:
• Soporte completo en español - Cambie entre English y Español en Configuración o desde la pantalla de inicio
• El asistente de IA ahora responde en su idioma preferido
• Navegación mejorada de Centros Regionales con alternador Mapa/Lista
• Acceso rápido a Acerca de y Preguntas Frecuentes desde la pantalla de inicio
• Página de inicio rediseñada con diseño más limpio

MEJORAS:
• Mejor localización en toda la aplicación
• Navegación optimizada a información clave
• Vista de mapa mejorada como predeterminada para Centros Regionales
• Accesibilidad mejorada para familias hispanohablantes

¡Gracias por usar KiNDD para encontrar servicios de discapacidades del desarrollo en el Condado de Los Ángeles!
```

---

## App Store Description

### English (Primary)
```
KiNDD - Developmental Services Navigator

Find ABA therapy and developmental disability resources across Los Angeles County with KiNDD, your free guide to Regional Center services.

KEY FEATURES:

INTERACTIVE MAP
• View all 7 LA County Regional Centers with color-coded boundaries
• Find your assigned Regional Center based on your ZIP code
• Explore 370+ verified ABA therapy providers and resources

AI ASSISTANT
• Ask questions in English or Spanish
• Get personalized recommendations for your family's needs
• Learn about Early Intervention, insurance options, and more

SMART SEARCH
• Filter by therapy type: ABA, Speech, Occupational, Physical
• Search by insurance: Medi-Cal, private insurance, Regional Center funding
• Find resources by age group and diagnosis

LOCATION-BASED
• Automatically find resources near you
• Get driving directions to any provider
• Call or email providers directly from the app

BILINGUAL SUPPORT
• Full Spanish language support
• Switch languages anytime from Settings
• AI responses in your preferred language

WHO WE SERVE:
• Families of children with Autism Spectrum Disorder
• Individuals with developmental disabilities
• Caregivers seeking therapy resources
• Anyone navigating the Regional Center system

REGIONAL CENTERS COVERED:
• San Gabriel/Pomona (SG/PRC)
• Harbor (HRC)
• North Los Angeles County (NLACRC)
• Eastern Los Angeles (ELARC)
• South Central Los Angeles (SCLARC)
• Westside (WRC)
• Frank D. Lanterman (FDLRC)

FREE & PRIVATE:
• No registration required
• No subscriptions or fees
• Your location data stays on your device

KiNDD is designed to help LA County families navigate the complex world of developmental disability services. Whether you're just starting your journey or looking for additional resources, we're here to help.

Questions? Contact us at support@kinddhelp.com
```

### Spanish (Español)
```
KiNDD - Navegador de Servicios del Desarrollo

Encuentre terapia ABA y recursos de discapacidades del desarrollo en todo el Condado de Los Ángeles con KiNDD, su guía gratuita para servicios de Centros Regionales.

CARACTERÍSTICAS PRINCIPALES:

MAPA INTERACTIVO
• Vea los 7 Centros Regionales del Condado de LA con límites codificados por colores
• Encuentre su Centro Regional asignado según su código postal
• Explore más de 370 proveedores de terapia ABA y recursos verificados

ASISTENTE DE IA
• Haga preguntas en inglés o español
• Obtenga recomendaciones personalizadas para las necesidades de su familia
• Aprenda sobre Intervención Temprana, opciones de seguro y más

BÚSQUEDA INTELIGENTE
• Filtre por tipo de terapia: ABA, Habla, Ocupacional, Física
• Busque por seguro: Medi-Cal, seguro privado, fondos del Centro Regional
• Encuentre recursos por grupo de edad y diagnóstico

BASADO EN UBICACIÓN
• Encuentre automáticamente recursos cerca de usted
• Obtenga direcciones de manejo a cualquier proveedor
• Llame o envíe correo a proveedores directamente desde la aplicación

SOPORTE BILINGÜE
• Soporte completo en español
• Cambie de idioma en cualquier momento desde Configuración
• Respuestas de IA en su idioma preferido

A QUIÉN SERVIMOS:
• Familias de niños con Trastorno del Espectro Autista
• Personas con discapacidades del desarrollo
• Cuidadores que buscan recursos de terapia
• Cualquiera que navegue el sistema de Centros Regionales

CENTROS REGIONALES CUBIERTOS:
• San Gabriel/Pomona (SG/PRC)
• Harbor (HRC)
• North Los Angeles County (NLACRC)
• Eastern Los Angeles (ELARC)
• South Central Los Angeles (SCLARC)
• Westside (WRC)
• Frank D. Lanterman (FDLRC)

GRATIS Y PRIVADO:
• No se requiere registro
• Sin suscripciones ni tarifas
• Sus datos de ubicación permanecen en su dispositivo

KiNDD está diseñado para ayudar a las familias del Condado de LA a navegar el complejo mundo de los servicios de discapacidades del desarrollo. Ya sea que esté comenzando su camino o buscando recursos adicionales, estamos aquí para ayudar.

¿Preguntas? Contáctenos en support@kinddhelp.com
```

---

## Keywords (100 characters max each)

### English
```
ABA therapy,autism,developmental disability,Regional Center,Los Angeles,LA County,therapy,Medi-Cal
```

### Spanish
```
terapia ABA,autismo,discapacidad desarrollo,Centro Regional,Los Angeles,condado LA,Medi-Cal,terapia
```

---

## Promotional Text (170 characters max)

### English
```
Now with full Spanish language support! Find ABA therapy and developmental disability services across all 7 LA County Regional Centers. Free, no registration required.
```

### Spanish
```
¡Ahora con soporte completo en español! Encuentre terapia ABA y servicios de discapacidades del desarrollo en los 7 Centros Regionales del Condado de LA. Gratis.
```

---

## Subtitle (30 characters max)

### English
```
LA County ABA & Therapy Finder
```

### Spanish
```
Buscador de Terapia ABA en LA
```

---

## App Category
- **Primary:** Medical
- **Secondary:** Health & Fitness

---

## Screenshots Needed

### iPhone (6.7" - iPhone 15 Pro Max)
1. **Home Screen** - Show the redesigned home page with KiNDD logo, AI assistant card, quick actions
2. **Regional Centers Map** - Show the colorful map with all 7 RC boundaries and the List/Map toggle
3. **AI Chat** - Show a conversation with the AI assistant (in English)
4. **AI Chat Spanish** - Show a conversation in Spanish
5. **Provider List** - Show filtered list of providers with therapy types
6. **Language Settings** - Show the language toggle in Settings
7. **About Page** - Show the About KINDD page
8. **FAQ Page** - Show FAQ with expanded answer

### Screenshot Captions

| # | English | Spanish |
|---|---------|---------|
| 1 | Find developmental services near you | Encuentre servicios de desarrollo cerca de usted |
| 2 | Explore all 7 LA County Regional Centers | Explore los 7 Centros Regionales del Condado de LA |
| 3 | AI assistant answers your questions | El asistente de IA responde sus preguntas |
| 4 | Full Spanish language support | Soporte completo en español |
| 5 | Filter by therapy, insurance & more | Filtre por terapia, seguro y más |
| 6 | Switch languages anytime | Cambie de idioma en cualquier momento |
| 7 | Learn about resources available | Aprenda sobre los recursos disponibles |
| 8 | Get answers to common questions | Obtenga respuestas a preguntas comunes |

---

## Privacy Policy URL
```
https://kinddhelp.com/privacy
```

## Support URL
```
https://kinddhelp.com/support
```

## Marketing URL
```
https://kinddhelp.com
```

---

## App Review Information

### Demo Account
Not required - app does not require login

### Notes for Reviewer
```
KiNDD is a free resource finder for families seeking developmental disability services in Los Angeles County, California.

Key features to review:
1. HOME: Tap "Home" tab to see the redesigned homepage with quick actions
2. REGIONAL CENTERS: Tap "Regions" tab to see the map with List/Map toggle (defaults to Map)
3. LANGUAGE: Go to Settings > Language to switch between English and Spanish
4. AI CHAT: Tap the sparkle button to ask questions (responds in selected language)
5. ABOUT/FAQ: Scroll to bottom of Home and tap "About KINDD" or "FAQ"

Location permission is requested to find the user's assigned Regional Center based on their location. The app functions without location access using ZIP code search.

All provider data is publicly available information about ABA therapy providers in LA County.
```

### Contact Information
- **First Name:** [Your Name]
- **Last Name:** [Your Last Name]
- **Email:** support@kinddhelp.com
- **Phone:** [Your Phone]

---

## Pre-Submission Checklist

- [ ] Version updated to 1.3.0
- [ ] Clean build succeeds (Cmd+Shift+K, Cmd+B)
- [ ] Test on physical device
- [ ] Test language switching (English ↔ Spanish)
- [ ] Test AI responses in both languages
- [ ] Test Regional Centers Map/List toggle
- [ ] Test About and FAQ sheets from home
- [ ] Archive build (Product > Archive)
- [ ] Upload to App Store Connect
- [ ] Add all screenshots (English + Spanish)
- [ ] Add all descriptions and metadata
- [ ] Submit for review

---

## Version History

| Version | Date | Changes |
|---------|------|---------|
| 1.0.0 | - | Initial release |
| 1.1.0 | - | Added AI assistant |
| 1.2.0 | - | Enhanced map features |
| 1.3.0 | Jan 2026 | Spanish language support, improved navigation, About/FAQ access |
